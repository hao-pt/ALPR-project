...
ap.add_argument("-m", "--model", required = False, help = "Path to SVM pretrained model", default = "SVM.xml")
ap.add_argument("-c", "--classify", required= False, help = "Use SVM to classify or not", default = "1")
ap.add_argument("-o", "--option", required= False, help = "Show step by step", default = "0")
args = vars(ap.parse_args())

def classify_plate(plate_img, model_path):
...
def main(args):
    ...
    if args["classify"] == "1":
        ...
        for i, plate_image in enumerate(eqhist_plates):
            if classify_plate(plate_image, args["model"]):
